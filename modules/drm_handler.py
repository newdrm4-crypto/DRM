...
# =================== Concurrent Settings ===================
CONCURRENT_LIMIT = 3   # एक साथ कितने लिंक प्रोसेस होंगे
semaphore = asyncio.Semaphore(CONCURRENT_LIMIT)

# ============================================================
# यह फंक्शन एक single लिंक को प्रोसेस करेगा (पुराने for-loop का body)
# ============================================================
async def process_single_link(
    bot, m, link_info, count, b_name, channel_id, raw_text, raw_text2, quality, res,
    topic, caption, endfilename, thumb, vidwatermark, CR, cwtoken, cptoken, pwtoken,
    name1, name, namef, file_name, path, links, total_links
):
    async with semaphore:
        try:
            ...
            # ---------- Captions and name formatting (same as original) ----------
            if m.text:
                ...
                if topic == "/yes":
                    ...
                    if caption == "/cc1":
                        ...
                        cc = f'<b>{str(count).zfill(3)}.</b> {namef} [{res}p] .mkv'
                        cc1 = f'<b>{str(count).zfill(3)}.</b> {namef} .pdf'
                        cczip = f'<b>{str(count).zfill(3)}.</b> {namef} .zip'
                        ccimg = f'<b>{str(count).zfill(3)}.</b> {namef} .jpg'
                        ccm = f'<b>{str(count).zfill(3)}.</b> {namef} .mp3'
                        cchtml = f'<b>{str(count).zfill(3)}.</b> {namef} .html'

            # ---------- URL processing (VisionIAS, Classplus, etc.) ----------
            if "visionias" in url:
            ...
            progress_percent = (count / total_links) * 100
            Show = f"<i><b>Video Downloading</b></i>\n<blockquote><b>{str(count).zfill(3)}) {namef}</b></blockquote>"
            Show1 = f"<blockquote>🚀𝐏𝐫𝐨𝐠𝐫𝐞𝐬𝐬 » {progress_percent:.2f}%</blockquote>\n┃\n" \
                    f"┣🔗𝐈𝐧𝐝𝐞𝐱 » {count}/{total_links}\n┃\n" \
            ...
            if "drive" in url:
                ...
                if "cwmediabkt99" in url:
                    ...
                    await helper.run_shell(cmd_pdf)
                    await bot.send_document(chat_id=channel_id, document=f'{namef}.pdf', caption=cc1)
                    os.remove(f'{namef}.pdf')

            elif any(ext in url for ext in [".jpg", ".jpeg", ".png"]):
                ext = url.split('.')[-1]
                cmd_img = f'yt-dlp -o "{namef}.{ext}" "{url}" -R 25 --fragment-retries 25'
                await helper.run_shell(cmd_img)
                await bot.send_photo(chat_id=channel_id, photo=f'{namef}.{ext}', caption=ccimg)
                os.remove(f'{namef}.{ext}')

            elif any(ext in url for ext in [".mp3", ".wav", ".m4a"]):
                ext = url.split('.')[-1]
                cmd_audio = f'yt-dlp -o "{namef}.{ext}" "{url}" -R 25 --fragment-retries 25'
                await helper.run_shell(cmd_audio)
            ...
            return {"status": "success", "index": count}

        except Exception as e:
            await bot.send_message(channel_id, f'⚠️**Downloading Failed**⚠️\n**Name** =>> `{str(count).zfill(3)} {namef}`\n**Url** =>> {url}\n\n<blockquote expandable><i><b>Failed Reason: {str(e)}</b></i></blockquote>', disable_web_page_preview=True)
            return {"status": "failed", "index": count, "error": str(e)}

# ============================================================
# मुख्य drm_handler (अब Concurrent)
# ============================================================
async def drm_handler(bot: Client, m: Message):
    ...
    # ----- Get user inputs (batch name, channel, etc.) -----
    if m.document:
    ...
    for i in range(start_index - 1, total_links):
        ...
        if m.text:
            if "youtu" in url:
                ...
                if name1.strip():
                    ...
                    default_name = f"Video_{str(i+1).zfill(3)}"
                    name = default_name
                    namef = default_name
        else:
            if topic == "/yes":
                ...
                if endfilename == "/d":
                    ...
                    name = f'{str(i+1).zfill(3)}) {name1[:60]} {endfilename}'
        ...
        # Create task for this link
        task = asyncio.create_task(
            process_single_link(
                bot, m, links[i], i+1, b_name, channel_id, raw_text, raw_text2, quality, res,
                topic, caption, endfilename, thumb, vidwatermark, CR, cwtoken, cptoken, pwtoken,
                name1, name, namef, file_name, path, links, total_links
            )
        )
